...
def compress_mp3_file(mp3_file_path):
    ...
    output_file_path = "{}.mp3".format(os.path.splitext(basename(mp3_file_path))[0])
...
#    sys.exit(-1)
path = "/Users/michael/Downloads/new-hrm/"
mp3_files = [x for x in listdir(path) if x.endswith(".mp3")]
# mp3_files = filter(lambda x: x.endswith(".mp3"), sys.argv[1:])

print("\n Files to precess: {}".format(mp3_files))

for mp3_file in mp3_files:
    compress_mp3_file(path+mp3_file)
    os.remove(path+mp3_file)
